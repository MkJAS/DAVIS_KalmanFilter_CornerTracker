...
default_properties = struct(...
  ...
  'mu', [corner(1) corner(2)], ... % Center of ellipse (optional)
...
% m = sqrt(qchisq(conf,r)); % r is the number of dimensions (degrees of freedom)


  [x,y,z,ab] = getpoints(C,prop.clip);
  a = max(ab(1,:));
  b = max(ab(2,:));
  r_ellipse = [x y];
...
%   x and y.
function [x,y,z,ab] = getpoints(C,clipping_radius)

n=100; % Number of points around ellipse
p=0:pi/n:2*pi; % angles around a circle

[eigvec,eigval] = eig(C); % Compute eigen-stuff
ab = sqrt(eigval) * eigvec';
xy = [cos(p'),sin(p')] * ab; % Transformation
x = xy(:,1);
y = xy(:,2);
z = zeros(size(x));
